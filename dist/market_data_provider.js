...
let web3 = new Web3(new Web3.providers.HttpProvider(URL0, web3ProviderOptions));
...
async function getPricesCCXT(epochId, assets) {
        ...
        try {
            ...
            for (let i = 0; i < allRawData.length; i++) {
                ...
                tickersRet.forEach(tickerSymbol => {
                    ...
                    volsEx[tickerSymbol].push(populateQuoteVolume(allRawData[i][tickerSymbol]).quoteVolume || 0);
                });
            }
            ;
            ...
            let baseAltsPxs = tickersAltsToBase.map((altTicker, idx) => math.dot(pxsEx[altTicker], volsEx[altTicker]) / math.sum(volsEx[altTicker]));
...
async function main() {
    ...
    symbols = await Promise.all(ftsoSupportedIndices.map(async (idx) => await ftsoRegistry.methods.getFtsoSymbol(idx).call()));
    ...
    decimals = await Promise.all(ftsos.map(async (ftso) => parseInt((await ftso.methods.ASSET_PRICE_USD_DECIMALS().call()))));
...
//# sourceMappingURL=market_data_provider.js.map